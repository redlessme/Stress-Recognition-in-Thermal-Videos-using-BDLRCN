...
class Dataset_3DCNN(data.Dataset):
    ...
    def read_images(self, path, selected_folder, use_transform):
        ...
        for i in self.frames:
            image = Image.open(os.path.join(path, selected_folder, 'img_{:05d}.jpg'.format(i))).convert('L')
...
class Dataset_CRNN(data.Dataset):
    ...
    def read_images(self, path, selected_folder, use_transform):
        X = []
        target_path =sorted(list(os.listdir(os.path.join(path, selected_folder))))
        frame_len=len(self.frames) #28
        target_path=target_path[:frame_len]
        for i in range(len(target_path)):
            image=Image.open(os.path.join(path, selected_folder,target_path[i]))
            if use_transform is not None:
                image = use_transform(image)
...
class DecoderGRU(nn.Module):
    ...
    def forward(self, x_RNN):
        ...
        GRU_out, (h_n, h_c) = self.GRU(x_RNN, None)
        """ h_n shape (n_layers, batch, hidden_size), h_c shape (n_layers, batch, hidden_size) """
        """ None represents zero initial hidden state. RNN_out has shape=(batch, time_step, output_size) """

        # FC layers
        x = self.fc1(GRU_out[:, -1, :])   # choose RNN_out at the last time step
        x = F.relu(x)
        x = F.dropout(x, p=self.drop_p, training=self.training)
        x = self.fc2(x)
